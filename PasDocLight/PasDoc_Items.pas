{ @abstract(defines all items that can appear within a Pascal unit's interface)
  @created(11 Mar 1999)
  @cvs($Date: 2008-06-19 20:31:44 +0200 (czw, 19 cze 2008) $)
...
{$ifndef DONTUSETAGS}
    ...
    property AbstractDescription: string           s
...
{$endif}
    { full Declaration of the item, even if the FullDeclaration field is void
     - e.g. expand TPasEnum types and TPasCIO }
    function ExpandedDeclaration: string;
    
    { return the name preceded by its UnitName, e.g. "UnitName.FunctioName" }
...
{$endif}
  ...
  protected
    ...
    procedure Deserialize(const ASource: TStream); override;
  public
    /// true if the constant was declared as resourcestring, not as const
...
{$endif}
  ...
  { extends @link(TPasItem) to store anything about a unit, its constants,
    ...
    Note: Remember to always set @link(CacheAge) after
  ...
  public
    ...
    property InterfaceUsesUnits: TStringVector read FInterfaceUsesUnits;
    ...
    property SourceFileName: string read FSourceFilename write FSourceFilename;
    property SourceFileAge: integer read FSourceFileAge write FSourceFileAge;
    ...
    { If WasDeserialized then this specifies the datetime
      ...
      don't use it. }
    property CacheAge: integer read FCacheAge write FCacheAge;
  ...
  public
    procedure SortBySourceFileName;
    procedure SortByDirectory;
...
const
  ...
  CIO_NAMES: array[TCIOType] of string = (
    'class',
    'dispinterface',
    'interface',
    'object',
...
uses ProjectCommons, PasDoc_Utils, PasDoc_Tokenizer;

function CompareUnitsItemsByDirectory(PItem1, PItem2: Pointer): Integer;
var P1,P2: string;
begin
  if (TObject(PItem1).ClassType = TPasUnit) and
     (TObject(PItem2).ClassType = TPasUnit) then begin
     P1 := ExtractFilePath(TPasUnit(PItem1).SourceFileName);
     P2 := ExtractFilePath(TPasUnit(PItem2).SourceFileName);
     Result := CompareText(P1,P2);
     if Result=0 then
       Result := CompareText(TPasUnit(PItem1).SourceFileName,TPasUnit(PItem2).SourceFileName);
   end else
     Result := CompareText(TPasItem(PItem1).Name, TPasItem(PItem2).Name);
end;

function CompareUnitsItemsBySourceFileName(PItem1, PItem2: Pointer): Integer;
begin
  if (TObject(PItem1).ClassType = TPasUnit) and
     (TObject(PItem2).ClassType = TPasUnit) then
     Result := CompareText(TPasUnit(PItem1).SourceFileName,TPasUnit(PItem2).SourceFileName) else
     Result := CompareText(TPasItem(PItem1).Name, TPasItem(PItem2).Name);
end;
...
begin
  ...
  FInterfaceUsesUnits := TStringVector.Create;
end;
...
begin
  ...
  if (InterfaceUsesUnits <> nil) and (ssUsesClauses in SortSettings) then
    InterfaceUsesUnits.Sort;
end;
...
begin
  ...
  FInterfaceUsesUnits.LoadFromBinaryStream(ASource);
  ...
  SourceFileAge := LoadDoubleFromStream(ASource);}
end;
...
begin
  ...
  FInterfaceUsesUnits.SaveToBinaryStream(ADestination);
  ...
  SaveDoubleToStream(SourceFileAge, ADestination); }
end;
...
function VisToStr(const Vis: TVisibility): string;
begin
  result := StringReplaceAll(VisibilityStr[Vis], ' ', '');
end;
...
procedure TPasUnits.SortByDirectory;
begin
  Sort(@CompareUnitsItemsByDirectory);
end;

procedure TPasUnits.SortBySourceFileName;
begin
  Sort(@CompareUnitsItemsBySourceFileName);
end;

function GetNextLine(source: PAnsiChar; out next: PAnsiChar): string;
begin
  next := source;
...
function TPasUnit.UnitDescription(OnlyFirstLine: boolean): string;
var P: PAnsiChar;
    line,res: string;
begin
  result := trim(RawDescriptionInfo.Content);
  if result<>'' then begin
    result[1] := NormToUpper[result[1]];
...
procedure TPasConstant.Deserialize(const ASource: TStream);
begin
  inherited;
  ASource.Read(IsResourceString,sizeof(IsResourceString));
end;

procedure TPasConstant.Serialize(const ADestination: TStream);
begin
  inherited;
  ADestination.Write(IsResourceString,sizeof(IsResourceString));
end;

function TPasItem.ExpandedDeclaration: string;
var anc: string;
    i: integer;
begin
  if InheritsFrom(TPasEnum) then
  with TPasEnum(self) do begin
    result := Name+' = (';
    for i := 0 to Members.Count-1 do begin
      result := result+TPasItem(Members[i]).Name;
  ...
  if InheritsFrom(TPasCIO) then begin
    result := result+' = '+CIO_NAMES[TPasCio(self).MyType];
    anc := TPasCio(self).FirstAncestorName;
    if anc<>'' then
      result := result+'('+anc+')';
  end;
end;
