(*
  ...
  @author(Arnaud Bouchez - (arnaud@bouchez.info) some speed up)
...
type
  ...
  private
    ...
    procedure _SetStringObject(const _key: String; const s: String; set_s: Boolean;
  ...
  public
    ...
    procedure SetString(const _key: String; const data: String);
    procedure SetStringObject(const _key: String; const s: String; p: Pointer);
    function GetObject(const _key: String): Pointer;
    function GetString(const _key: String): String;
    procedure GetStringObject(const _key: String; var s: String; var p: Pointer);
...
function THash.Hash(const key: String): Integer;
var
  i: Integer;
begin
  Result:=0;
...
procedure THash._SetStringObject(const _key: String; const s: String; set_s: Boolean;
                                  p: Pointer; set_p: Boolean);
var h: Integer;
    he, anker: PHashEntry;
begin
  if FeldBelegt>FeldMaxValue then
    MakeHashLarger(1);
  h:=Hash(_key);
  he:=Feld^[h and FeldMaxValue];
  anker:=he;
  ...
  //not found; a new key must be created
  Getmem(he,sizeof(he^));
...
procedure THash.SetStringObject(const _key: String; const s: String; p: Pointer);
begin
...
procedure THash.SetString(const _key: String; const data: String);
begin
